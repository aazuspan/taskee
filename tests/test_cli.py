...
def test_log_command(notifier, watch_for, cli, mock_pending_task, mock_running_task):
    """The `log` command should run and log events.
    ...
    with (
        ...
        result = cli.invoke(taskee, ["log", *watch_for, *args])
...
def test_dashboard_command(
    ...
    """The `dashboard` command should run and log events.
    ...
    with (
        ...
        args = ["--interval-mins", UPDATE_INTERVAL, "--notifier", notifier]
        result = cli.invoke(taskee, ["dashboard", *watch_for, *args])
...
def test_mode_command_with_key(
    mode,
    cli,
    tmpdir,
    mock_task_list,
    ...
    """The mode commands should accept and use service credentials."""
    ...
    with patch("ee.data.listOperations") as listOperations:
        ...
        result = cli.invoke(taskee, [mode, "--private-key", key_file])
...
def test_mode_command_handles_crash(mode, cli, mock_native_notifier):
    """The mode commands should send an error notification on crash."""
    with patch("ee.data.listOperations") as listOperations:
        listOperations.return_value = []
        result = cli.invoke(taskee, [mode, "--notifier", "all"])
...
@pytest.mark.parametrize("mode", ["log", "dashboard"])
@pytest.mark.usefixtures("_keyboardinterrupt_on_sleep")
def test_start_command_is_deprecated(mode, cli, mock_task_list):
    """The legacy `start` command should still run but warn about deprecation."""
    with patch("ee.data.listOperations") as listOperations:
        listOperations.return_value = [task.model_dump() for task in mock_task_list]
        result = cli.invoke(taskee, ["start", mode])

    assert result.exit_code == 0, result.output
    assert "deprecated" in result.output
    assert f"taskee {mode}" in result.output
