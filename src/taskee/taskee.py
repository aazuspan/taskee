...
class Taskee:
    """The Taskee object is the primary interface to the taskee library. It connects
    the task manager, which tracks tasks and events, to the dispatcher, which
    distributes notifications. The Taskee object is the only direct point of contact
    with Earth Engine."""
    ...
    def _update(self, watch_for: set[type[events.Event]]) -> tuple[events.Event, ...]:
        """Update tasks and return any events that occured. Dispatch notifications for
        events of interest."""
